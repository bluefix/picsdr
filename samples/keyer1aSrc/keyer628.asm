;**********************************************************************
;                                                                     *
;   KEYER628.ASM  by W.Buescher (DL4YHF)                              *
;                                                                     *
;                                                                     *
;   ELBUG-firmware for PIC16F628                                      *
;                                                                     *
...
;     07/2002: Made a variant of KEYER1(!) for the PIC16F628.         *
;           Project name: keyer628.asm .                              * 
;           - All RAM locations changed                               *
;           - CONFIG word changed ( _RC_OSC  ->  _ER_OSC_CLKOUT)      *
;           - 128 byte EEPROM instead of 64 now used completely       *
;           - lots of other mods because PIC16F84 and F628 are not as *
;             compatible as first hoped, among them:                  *  
;              RC-oscillator, lousy EEPROM access, comparators off,   *
;     04/2004: Switched from EXTERNAL to INTERNAL RC-Oscillator .     *
;           (Caution: the PIC16F628 had an "ER"-Oscillator, no cap    *
;            required; the PIC16F628A has the old "ext RC"-Osc again, *
;            which again requires an external capacitor. Too bad !    *
;            So we have to live with the internal RC which both have) *
;     09/2005: Caution again: The PIC16F628A is *NOT* compatible with *
;            the PIC16F628 ! Google for PIC16F62XA/16F648A Migration. *
;            This PIC-QRP-Keyer seems not to be affected by most of   *
;            these differences, BUT: THE BITS IN THE CONFIG WORD HAVE *
;            BEEN CHANGED, SO YOU WILL MOST LIKELY NEED A DIFFERENT   *
;            FIRMWARE FOR THE 'NEW' PIC16F628A  !                     *
;                                                                     *
;**********************************************************************
;  Usage of this file:                                                *
;    - Compile with Microchip's MPLAB, using project file KEYER1.PJT  *
;    - program the generated .HEX-Files mit "PIP02" into a PIC16F628  *
...
;     Contact me via Internet:      DL4YHF@qsl.net,  DL4YHF@aol.com   *
;                                                                     *
;**********************************************************************
;                                                                     *
...
; EX(<2004-04):  __CONFIG   _CP_OFF & _WDT_OFF & _PWRTE_ON &_ER_OSC_CLKOUT & _LVP_OFF & _BODEN_OFF
; 2004-04-13: changed from EXTERNAL RC-OSCILLATOR   to   INTERNAL, SLOW, RC-OSCILLATOR (37kHz)
 __CONFIG   _CP_OFF & _WDT_OFF & _PWRTE_ON & _INTRC_OSC_NOCLKOUT & _LVP_OFF & _BODEN_OFF & _MCLRE_OFF
; See Notes on the clock oscillator below, and in the datasheet. The INTERNAL OSCILLATOR
; must be configured for 37 kHz operation by software; using bit OSCF in the PCON register.
...
; Clock Oscillator
; ----------------
;  The PIC WAS running in "low power mode" with 32.768kHz crystal.
...
#define SWI_TEST_POTI     1 ; 1: Test Poti Timing (with oscilloscope)
...
; Note: PORTA has no internal pull-ups. Unused INPUTS should be tied to Vcc
;       with an external pull-up to avoid floating, which draws add'l current.
...
#define IOP_AUDIO_OUT  PORTA, 3    ; sidetone (piezo speaker, also "signals")
 ; ex: #define IOP_COUNTER    PORTA, 4    ; frequency counter only with 4 MHz XTAL
#define IOP_I2C_CLK    PORTA, 4    ; serial clock for I2C bus (optional)
...
#define  EEPROM_ADR_CW_SPEED   0x7F  ; EEPROM location for "speed" (16F84: 0x3F)
#define  EEPROM_ADR_MSG1_START 0x7E  ; Start of recorded message   (16F84: 0x3E)
                                     ; (used as index, runs DOWN to zero)

; RAM memory (general purpose registers, unfortunately not the same for PIC16F84 & PIC16F628)
;     old:    PIC16F84:  RAM from 0x0C..0x4F
;     new:    PIC16F628: RAM from 0x20..0x7F   (96 bytes, 0x20.. only accessable in Bank0)
...
w_temp        EQU     0x70        ; variable used for context saving (16F84: 0x0C)
status_temp   EQU     0x71        ; variable used for context saving (16F84: 0x0D)


; Keyer Variables
KY_state      EQU     0x2E        ; keyer state (16F84: 0x0E.. nn)
CW_timing     EQU     0x2F        ; duration of a "dot" (for keyer+generator etc)
CW_t_cnt      EQU     0x30        ; CW (keyer-) timer
KY_mem_index  EQU     0x31        ; Keyer memory index for record + replay
KY_flags      EQU     0x32        ; Keyer flags... etc
...
KY_char_latch    EQU  0x33        ; Buffer for decoding the SENT character
KY_decoded_char  EQU  0x34        ; output of the CW decoder (ready chars)
KY_count_elements EQU 0x35        ; Counter for dashes+dots in a character
KY_dot_counter   EQU  0x36        ; counts number of "dot times" for long delays
PT_counter       EQU  0x37        ; Poti-Scaler (counts main loops)

NoActivityTimer  EQU  0x38        ; timer for "no activity" detection
...
OP_flags         EQU  0x39        ; option flags (saved in EEPROM)..
...
NR_hundreds      EQU  0x3A        ; three-digit-number (unpacked BCD)
NR_tens          EQU  0x3B
NR_ones          EQU  0x3C

bStorageData     EQU  0x3D        ; replaces EEDATA which is in Bank1 of the 16F628 (!)

Msg2_Start       EQU  0x40        ; start of message Nr. 2 (volatile!) 16F84: 0x2D
Msg2_End         EQU  0x6F        ; end  of  message Nr. 2
RAM_MSG2_LENGTH  EQU  (Msg2_End - Msg2_Start + 1)

;**********************************************************************
; Initial contents of DATA EEPROM:
 ;  ex:  org (H'2100'+EEPROM_ADR_MSG1_START-14-16-16-4-10-63)  
        ; When upgrading from an older version, MPLAB 6.40 moaned :
        ; > Warning[220] C:\PIC\QRP_KEYER\KEYER628.ASM 347 : 
        ; >   Address exceeds maximum range for this processor.
        ; > Reason may be the default radix which seems to be HEX now.
   org (H'2100'+EEPROM_ADR_MSG1_START-d'14'-d'16'-d'16'-d'4'-d'10'-d'63')
   ; Initialize EEPROM Data with a small partitioned Test Message 
   ; (this is "Message #1", reverse address order!)
   ;     PIC16F84:  EEPROM_ADR_MSG1_START = 0x3E
   ;     PIC16F628: EEPROM_ADR_MSG1_START = 0x7E
   ; This will be overwritten as soon as the user records his own message.
	de 0 ; termination with null byte

        ; 6th part of message#1: 14 byte    "ur rst 599/<serial number>"
        ...
        ; 5th part of message#1: 16 byte    "test de  dl4yhf .-.-."
        de CW_CHR_AR       
        ...
        ; 4th part of message#1: 16 byte    "de dl4yhf pse  k"
        de CW_CHR_K
        de CW_CHR_SPACE+10
        ...
        ; 3rd part of message#1: 4 byte     "73 <advance serial number>"
        de CW_CHR_ANN
        de CW_CHR_3,CW_CHR_7
        de CW_CHR_EOM
  
        ; 2nd part of message#1: 10 byte     "ur nr <number> <number>"
        ...
        ; 1st part of prerecorded message#1:
        ; "QRP PIC keyer by dl4yhf = hold button pressed 1 sec to record = .-.-."
        ;  12345678901234567890123456789012345678901234567890123456789012345
        ;           1         2         3         4         5         6
        de CW_CHR_AR, CW_CHR_SPACE+5
        de CW_CHR_D, CW_CHR_R, CW_CHR_O, CW_CHR_C, CW_CHR_E, CW_CHR_R
        de CW_CHR_SPACE+5
        de CW_CHR_O, CW_CHR_T
        de CW_CHR_SPACE+5
        de CW_CHR_C, CW_CHR_E, CW_CHR_S, CW_CHR_SPACE+5, CW_CHR_1
        de CW_CHR_SPACE+5
        de CW_CHR_D, CW_CHR_E, CW_CHR_S, CW_CHR_S, CW_CHR_E, CW_CHR_R, CW_CHR_P
        de CW_CHR_SPACE+5
        de CW_CHR_N, CW_CHR_O, CW_CHR_T, CW_CHR_T, CW_CHR_U, CW_CHR_B
        de CW_CHR_SPACE+5
        de CW_CHR_D, CW_CHR_L, CW_CHR_O, CW_CHR_H
        de CW_CHR_SPACE+5
        de CW_CHR_SEPAR
        de CW_CHR_SPACE+5
        de CW_CHR_F, CW_CHR_H, CW_CHR_Y
        de CW_CHR_4, CW_CHR_L, CW_CHR_D
        de CW_CHR_SPACE+5                  
        de CW_CHR_Y, CW_CHR_B
        de CW_CHR_SPACE+5
        de CW_CHR_R, CW_CHR_E, CW_CHR_Y, CW_CHR_E, CW_CHR_K
        de CW_CHR_SPACE+5
        de CW_CHR_C, CW_CHR_I, CW_CHR_P
        de CW_CHR_SPACE+5
        de CW_CHR_P, CW_CHR_R, CW_CHR_Q
...
reset:  ;----- Hardware initialization -----
        ...
        ; Switch the internal RC oscillator (in a PIC16F628) to 37 kHz. 
        ; Even this ridiculously simple task can go wrong if you trust a lousy wrong datasheet.
        bsf     STATUS, RP0       ;! set RP0 enables access to PCON register ... baaah
        bcf     PCON, OSCF        ;! clear bit "OSCF" in "PCON" register for 37 kHz oscillator
        bcf     STATUS, RP0       ;! reset RP0 for normal operation... ISN'T THIS TERRIBLE ?!?
        ; GRUMBLE again about the rotten documentation by Microchip:
        ; PCON is not at address 0x0C as they tell you in chapter 3.2.2.6 , 
        ;      instead it is at address 0x8E so be carefull with the f***** register bank ! !

        ; turn off unused function blocks ... 
        bsf     STATUS, RP0       ;! set RP0 enables access to VRCON register ... baaah again
        clrf    VRCON             ;! turn voltage reference module off (16F628, register bank 1 ! !)
        bcf     STATUS, RP0       ;! reset RP0 for normal operation... and to access CMCON, holy s***
        movlw   0x07              ; turn comparators off and          (16F628)  
        movwf   CMCON             ; enable pins for digital I/O       (16F628)
        ; initialize port directions
        bcf     STATUS, RP1       ; default value: use register bank 0+1 only
  ...
  ;     bsf     IOP_COUNTER    ;! ; input (attention: PORTA has no internal pull-up)
        bcf     IOP_I2C_CLK    ;! ; output
...
;=======================================================================
       ...
       ; A "conversion table" is included in the poti-read-routine !
       ; Maximum allowed time in this routine : see end of routine
ReadPoti:
        bcf     INTCON, GIE       ;01 disable IRQs while we are in register bank 1
                                  ; (and to prevent bad poti readings !)
        bcf     IOP_POTI1         ;02 clear Poti pin output latch to discharge
...
;  Input parameters:
;    bStorageData   contains byte to be written (was once EEDATA)
...
#define  L_save_eep_timer KY_char_latch
        ...
        ; write to EEPROM data memory as explained in PIC16F84 + 16F628 data sheet
        ; EEDATA and EEADR must have been set before calling this subroutine
        ; (optimized for the keyer-state-machine).
        ; CAUTION CAUTION CAUTION: What the lousy datasheet DS40300B wont tell you:
...
SaveInEEPROM:
         movwf   EEADR                 ; write into EEPROM address register (BANK0 ONLY FOR 16F84)
         bcf     INTCON, GIE           ; disable INTs
         movf    bStorageData,w        ; w := bStorageData
         movwf   EEDATA                ; EEDATA(in BANK0) := w  (ONLY WORKS FOR 16F84, NOT F628 !)
...
SaveInEEPROM:    ; save <bStorageData> in EEPROM[<w>]
         bcf     INTCON, GIE           ; disable INTs
...
#endif ; __16F84
#ifdef __16F628
        ; grrrrrrr ... EEDATA and EEADR have been moved from Bank0(16F84) to Bank1(16F628),
        ;              but the rotten example from the datasheet telling you to switch to 
        ;              bank0 to access EEDATA which is rubbish (DS40300B page 93 example 13-1).
        ; Caution: Under MPLAB 5.50, the EEPROM in the PIC16F628 is not emulated properly.
ReadFromEEPROM:
        bcf     INTCON, GIE             ;01 disable INTs
        bsf     STATUS, RP0             ;02 Bank1 for ***ALL*** EEPROM registers in 16F628 (!!!)
        movwf   EEADR                   ;03 write into EEPROM address register
        bsf     EECON1, RD              ;04 set "Read"-Flag for EEPROM
                                        ;   why is EECON1.RD not cleared in MPLAB-sim ?!?
        movf    EEDATA, w               ;05 read byte from EEPROM latch
        bcf     STATUS, RP0             ;06 normal access to Bank0
        bsf     INTCON, GIE             ;07 re-enable interrupts
        return                          ;08+09 back to caller
#endif ; __16F628
 ; end ReadFromEEPROM
...
ReadFM2: ; note that <w> still contains the not-decremented index.
        ...
        ;  For PIC16F628, be careful with FSR indexed addresses
        ;                 to take STATUS.IRP into account .
        bcf     STATUS, IRP             ; indirect access 0x00..0xFF via FSR
        addlw   Msg2_Start              ; add base address of buffer in RAM
...
AppendToMessage:
        ...
        movwf   bStorageData            ; data to be written -> bStorageData (not EEDATA, may be bank1 !!)
        ...
        call    SaveInEEPROM            ; save <bStorageData> in <w>
App_di: decfsz  KY_mem_index,F          ; address for next recorded letter
        ...
        bsf     KY_flags,ky_msg_full    ; set "message buffer full" - flag
                                        ;  (will generate "f" on pizeo with system message pitch)
...
Append_msg2     ; if "Message2" is used instead of "Message1".
        ...
        bcf     STATUS, IRP             ; indirect access 0x00..0xFF via FSR
        movwf   FSR                     ; store in "data memory pointer"
        movf    bStorageData,w          ; get write data from temporary var (to use EEDATA is impossible for F628 !)
